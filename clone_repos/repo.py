...
class Repo:
    ...
    def _editable_install(self) -> None:
        ...
        if any(t in installed for t in targets):
            ...
            cmd = f"""{sys.executable} -m pip install {'--user' if not self.editable_non_user else ''} {'--break-system-packages' if self.break_system_packages else ''} --use-pep517 --editable '{self.target}'"""
            click.echo(f"{self.name}: running '{cmd}'")
            proc = subprocess.Popen(shlex.split(cmd))
            proc.wait()
            if proc.returncode != 0:
